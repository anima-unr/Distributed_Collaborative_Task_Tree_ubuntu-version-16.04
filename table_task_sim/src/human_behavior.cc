...
void HumanBehavior::UpdateActivationPotential() {
 ...
 // ROS_WARN_NAMED("HumanBehavior", "%s: activation_potential: [%f]", object_.c_str(), state_.activation_potential );
  //ROS_INFO("%s in UpdateActivationPotential\n\n\n",obj_name_.c_str());
...
bool HumanBehavior::Precondition() {
  ...
  if (obj_started_ == 1 && check == 0){
    node_ = state_.owner.node;
    robot_id_ = state_.owner.robot;
    check = 1;
  }

  //ROS_INFO("%s hellooooooooooo %s\n\n\n\n\n",object_.c_str(),obj_name_.c_str());
  //ROS_INFO("%s %d in Precondition\n\n\n",obj_name_.c_str(),obj_started_);
//
   // if (object_.c_str() == obj_name_.c_str()){
  //ROS_INFO("%s %d in Precondition\n\n\n",obj_name_.c_str(),obj_started_);
  if (strcmp(obj_name_.c_str(),object_.c_str()) == 0){
      ROS_INFO("%s and %s are equal\n\n\n\n", object_.c_str(),obj_name_.c_str());
    }

    //if(state_.peer_active == 1 && obj_started_ ==1 && node_ == state_.owner.node  && robot_id_ == state_.owner.robot ) {
  if(state_.peer_active == 1 && obj_started_ ==1 && strcmp(obj_name_.c_str(),object_.c_str()) == 0) {

      ROS_ERROR("COLLISION IS HAPPENING FOR OBJECT = %s robot id %d~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",object_.c_str(),robot_id_);
       if(state_.peer_active_robot_0 == 1){
        ROS_ERROR("COLLISION IS HAPPENING FOR OBJECT = %s with Robot 0 - PR2~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n\n\n",object_.c_str());
        state_.collision_pr2 = true;
      }
      if(state_.peer_active_robot_1 == 1){
        ROS_ERROR("COLLISION IS HAPPENING FOR OBJECT = %s with robot 1 - Baxter~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n\n\n",object_.c_str());
        state_.collision_baxter = true;
      }
...
bool HumanBehavior::ActivationPrecondition() {
  ...
  bool lock_okay;
    ...
    ROS_ERROR("%s %d %s",object_.c_str(),state_.peer_active,name_->topic.c_str());
 ...
 // first attempt which did not work.....
  ...
  ROS_ERROR("%s %d %d %s",object_.c_str(),state_.peer_active,state_.peer_done,name_->topic.c_str() );
...
// second attempt
  ...
  void HumanBehavior::ObjStatusCallback( robotics_task_tree_msgs::ObjStatus msg)
  {
    ...
    ROS_INFO("%s in ObjStatusCallback\n\n\n",obj_name_.c_str());
